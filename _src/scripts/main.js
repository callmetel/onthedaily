...
function learnMore(state) {
  ...
  if (state === 'open') {
    ...
    learnTL.to($overlay, 1, {backgroundColor: 'rgba(18, 18, 18, .8)'}, '-=1.0')
      .fromTo($learnWrap, 1.5, {y: 45}, {y: 0}, '-=0.5');
    $skipBtn.css({
      'opacity': 0,
      'visibility': 'hidden'
    });
...
function storyText(state) {
  ...
  storyTL
    .to($overlay, 1, {backgroundColor: 'rgba(18, 18, 18, .4)'})
    .fromTo($titleOne, 2, {y: 70}, {y: 10}, '-=0.7')
    .fromTo($titleOne, 1.5, {alpha: 0, visibility: 'hidden'}, {alpha: 1, visibility: 'visible'}, '-=1.5')
    .to($titleOne, 1.5, {y: -20}, '+=0.7')
    .to($titleOne, 1, {alpha: 0.6}, '-=1.0')
    .fromTo($titleTwo, 2, {y: 25}, {y: -25}, '-=1.7')
    .fromTo($titleTwo, 1, {alpha: 0, visibility: 'hidden'}, {alpha: 1, visibility: 'visible'}, '-=1.5');
